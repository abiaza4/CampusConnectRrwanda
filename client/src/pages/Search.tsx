...
export default function Search() {
  ...
  return (
    <div>
      ...
      <section className="relative bg-navy/90 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">{t("search.title")}</h1>
          <p className="text-lg text-white/80 max-w-2xl mx-auto mb-8">{t("search.description")}</p>
          <div className="max-w-2xl mx-auto relative">
            <input
              ...
              className="w-full px-6 py-4 pl-12 rounded-full text-navy focus:outline-none focus:ring-2 focus:ring-gold/50"
            />
            ...
            {results.map((uni) => (
              <ScrollReveal key={uni.id}>
                <div className="bg-white rounded-xl shadow-md p-6 hover:shadow-lg transition-shadow">
                  <h3 className="text-xl font-bold text-navy mb-2">{uni.name}</h3>
                  <div className="flex items-center gap-2 text-gray-500 mb-3">
                    <MapPin className="w-4 h-4" />
                    <span className="text-sm">{uni.city}</span>
                    <Badge variant="secondary" className="ml-2">{uni.type}</Badge>
                  </div>
                  <p className="text-gray-600 mb-4">{uni.description.substring(0, 150)}...</p>
